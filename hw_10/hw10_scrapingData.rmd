...
## Save data
write.table(imbd_df2, "./data/imbd_top250movies_summary.tsv",
...
```{r}
ggplot(imbd_df2) +
      geom_histogram(aes(x = yearRelease))
ggplot(imbd_df2) +
      geom_point(aes(x = yearRelease, y = Rating))
ggplot(imbd_df2) +
      geom_point(aes(x = no.Votes, y = Rating)) +
      geom_smooth(aes(x = no.Votes, y = Rating))
```
